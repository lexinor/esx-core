...
function OpenManageSalaryMenu(society)
	...
	ESX.TriggerServerCallback('esx_society:'..ecallback, function(jf)
		...
		for i=1, #jf.grades, 1 do
			...
			elements[#elements+1] = {
				icon = "fas fa-wallet",
				title = ('%s - <span style="color:green;">%s</span>'):format(gradeLabel, TranslateCap('money_generic', ESX.Math.GroupDigits(jf.grades[i].salary))),
...
function OpenManageGradesMenu(society)
	...
	ESX.TriggerServerCallback('esx_society:'..ecallback, function(jf)
		...
		for i=1, #jf.grades, 1 do
			local gradeLabel = (jf.grades[i].label == '' and jf.label or jf.grades[i].label)
			elements[#elements+1] = {icon = "fas fa-wallet", title = ('%s'):format(gradeLabel), value = jf.grades[i].grade}
		end
